...
@app.route("/")
def index():
    """Root route (API info only — no HTML template required)"""
    return jsonify({
        "message": "🚀 Travel Package Recommender API is running!",
        "endpoints": {
            "POST /api/recommend": "Get travel package recommendations",
...
if __name__ == "__main__":
    ...
    if recommender.df is not None:
        ...
        print("❌ Dataset not loaded. Please check if packagedata_with_id.csv exists.")
    ...
    # Render/production safe configuration
